/**
 * 世界运转日志 —— 「世界背面」底部那张卡里滚过去的那些行。
 *
 * 这一屏讲的是世界的背面：事件怎么汇成回响、回响怎么又成了下一枚命运的因。
 * 星图给的是结果的形状，这张卡给的是它还在跑这件事 —— 世界不是等你来了才开
 * 始算，你翻到背面只是看见了它一直在算。
 *
 * 所以每行都是两截：
 *
 ...
 * 卡里同时显示几行。
 *
 * 九行 × 18 行高，加上表头和内边距，整张卡差不多是屏高的四分之一 —— 一屏
 * 812，一格 203。行数和行高一起决定卡多高，改一个就得回头看另一个。
 */
export const RUNTIME_WINDOW = 9;
/**
