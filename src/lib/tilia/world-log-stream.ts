 ...
 *
 * 那张卡（世界一直在算）本来滚的是手写的词库（`world-runtime.ts`）：它要表达
 * 的是「世界不是等你来了才开始算」，词是假的、意思是真的。现在有了真的账可
 * 看，就让它滚真的 —— 同一张卡、同一种滚法，只是内容换成后端此刻正在跑的行。
 *
 * ## 为什么要绕一道自己的接口
 *
 ...
 * 出时 `src/app/api` 整个会被搬走（见 `scripts/deploy-pages.sh`）。连不上就退回
 * 手写那套词库接着滚 —— 讲的人不会因为不在内网就少一张卡。
 */
